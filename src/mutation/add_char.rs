...
impl AddCharIter {
    ...
    pub(super) fn next(&mut self) -> Option<&[u8]> {
        ...
        if self.idx >= chars_len {
            return None
        }
        let prev_letter = self.idx.checked_sub(1)
        ...
        let next_char = mem::replace(&mut self.chars[self.idx], b'a');
        self.idx += 1;
        if let Some(next) = self.chars.get_mut(self.idx) {
            *next = next_char;
            Some(&self.chars)
        } else {
            self.chars[self.idx - 1] = next_char;
            None
        }
    }
